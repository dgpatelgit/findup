...
class FsObject:
  ...
  def insert(self, id, type, fullPath, sizeInBytes, commitImmediately):
    ...
    query = "INSERT INTO fsobject(id, scan_id, type, full_path, state, size_in_bytes, content_hash, created_timestamp, modified_timestamp) " \
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    data = (id, self.scanId, type, fullPath, self.FSOBJECT_STATE_PENDING, sizeInBytes, "", timestamp, timestamp)
  ...
  def markDuplicateFiles(self):
    ...
    query = f"UPDATE fsobject SET state = {self.FSOBJECT_STATE_UNIQUE_BY_SIZE} WHERE scan_id = {self.scanId} AND size_in_bytes IN (SELECT size_in_bytes FROM fsobject WHERE scan_id = {self.scanId} GROUP BY size_in_bytes HAVING COUNT(*) = 1)"
    ...
    query = f"SELECT id, full_path FROM fsobject WHERE scan_id = {self.scanId} AND state = {self.FSOBJECT_STATE_PENDING}"
    ...
    query = f"UPDATE fsobject SET state = {self.FSOBJECT_STATE_DUPLICATE_BY_HASH} WHERE scan_id = {self.scanId} AND state = {self.FSOBJECT_STATE_HASH_COMPUTED} AND content_hash IN (SELECT content_hash FROM fsobject WHERE scan_id = {self.scanId} GROUP BY content_hash HAVING COUNT(*) > 1)"
    self.db.execDelete(query, None, True)

    # 4. Mark all remaining files as unique whose size match but hash does not match
    query = f"UPDATE fsobject SET state = {self.FSOBJECT_STATE_UNIQUE_BY_HASH} WHERE scan_id = {self.scanId} AND state = {self.FSOBJECT_STATE_PENDING}"
    self.db.execDelete(query, None, True)
